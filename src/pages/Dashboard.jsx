...
import { Target, Plus, Calendar, Rocket, Archive, ArchiveRestore, ChevronDown, ChevronUp, ChevronRight, Trash2, Search, X, FileDown } from 'lucide-react';
...
function ArchivedReleaseRow({ initiativeId, rp }) {
    const statusClass =
        rp.status === 'Pending'
                    ...
                    : rp.status === 'Development'
                        ...
                        : rp.status === 'Ready for Release'
                            ...
                            : rp.status === 'Released'
                                ...
                                : 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400';
    const targetDate = rp.external_release_date || rp.dev_end_date || rp.planning_end_date;
    return (
        <div className="flex items-center gap-3 pl-12 pr-6 py-2 bg-blue-50/30 dark:bg-blue-900/10 border-l-4 border-l-blue-300/70 dark:border-l-blue-500/50">
            ...
            <Link
                ...
                className="text-xs font-medium text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:underline transition-colors truncate"
                title={rp.goal}
            >
                {rp.goal}
            </Link>
            <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full whitespace-nowrap ${statusClass}`}>
                {rp.status}
            </span>
            {targetDate && (
                <span className="flex items-center gap-1 text-[11px] text-slate-400 dark:text-slate-500 whitespace-nowrap">
                    <Calendar className="w-3 h-3" />
                    {format(new Date(targetDate + 'T00:00:00'), 'MMM d, yyyy')}
                </span>
            )}
        </div>
    );
...
function ArchivedInitiativeRow({ init, onUnarchive, onDelete }) {
    const [isExpanded, setIsExpanded] = React.useState(false);
    const releases = init.releasePlans || [];
    const hasReleases = releases.length > 0;
    return (
        <div>
            <div className="flex items-center gap-3 px-6 py-3 bg-slate-50/40 dark:bg-slate-800/20 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                {hasReleases ? (
                    <button
                        type="button"
                        ...
                        title={isExpanded ? 'Collapse releases' : 'Show releases'}
                    >
                ...
                <Link
                    ...
                    className="text-sm text-slate-500 dark:text-slate-400 line-through hover:text-blue-600 dark:hover:text-blue-400 hover:no-underline transition-colors truncate text-left"
                    title="View details"
                >
                    {init.name}
                </Link>
            ...
            {isExpanded && hasReleases && (
                <div className="divide-y divide-blue-100/40 dark:divide-blue-800/30">
                    {releases.map((rp) => (
                        <ArchivedReleaseRow key={rp.id} initiativeId={init.id} rp={rp} />
                    ))}
                </div>
            )}
        </div>
                        ...
                        <table className="w-full min-w-[960px] text-left text-sm">
                            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                                <tr>
                                    ...
                                    <th className="px-4 py-4 font-semibold text-slate-900 dark:text-slate-100">Initiative Name</th>
                                    <th className="px-4 py-4 font-semibold text-slate-900 dark:text-slate-100 w-[170px]">Phase</th>
                                    <th className="px-4 py-4 font-semibold text-slate-900 dark:text-slate-100 w-[140px]">Target Date</th>
                                    <th className="px-4 py-4 font-semibold text-slate-900 dark:text-slate-100 w-[140px]">Target Release</th>
                                    <th className="px-4 py-4 font-semibold text-slate-900 dark:text-slate-100 w-[110px]">Group</th>
                                    <th className="px-2 py-4 font-semibold text-slate-900 dark:text-slate-100 w-10"></th>
                                </tr>
                            </thead>
                            <SortableContext
